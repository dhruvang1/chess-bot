...
//static const int TTSize = 9999973; // close to 10M * 50 bytes per entry => 500 MB
//static const int TTSize = 40000003; // close to 40M * 24 bytes per entry => 1G
static const int TTSize = 19999999; // close to 20M * 50 bytes per entry => 1G
...
struct TTEntry {
    ...
    TTEntry(uint64_t hash, const string& move, int eval, int depth, int flag) {
       this -> hash = hash;
       this -> move = move;
       this -> eval = eval;
       this -> depth = depth;
